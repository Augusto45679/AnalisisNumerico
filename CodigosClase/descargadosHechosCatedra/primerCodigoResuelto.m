function primerCodigoResuelto
  ...
  for k=1:m
     ...
     kw(k+1)=k*dw;
     for j=1:N
      FI(j,2*k)  =cos(k*w0*(j-1)); % esto es lo de las bases
      FI(j,2*k+1)=sin(k*w0*(j-1)); % en las numeros impares de la matriz mete los seno y en las partes pares pone cosenos
     end
  end
  %FI
  %FI'*FI
